...
- **Список администраторов** — `ADMIN_IDS` ограничивает доступ к командам /check и /status. Если не задан — команды доступны всем
...
5. **ADMIN_IDS** (опционально) — список ID администраторов через запятую (например, `123456789,987654321`). Только они смогут использовать /check и /status. Если не задан — команды доступны всем.

6. **TELEGRAM_CHANNEL_ID** — опционально; сообщения в канал отправляются только в ответ на /check или /status.
